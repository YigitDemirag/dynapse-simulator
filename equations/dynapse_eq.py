...
def dynapse_eq():
    return{'model': '''
                    ...
                    dIsoma_mem/dt = (((Isoma_dpi_g_clip / Isoma_dpi_tau) *\
                    (Iin_clip + Isoma_pfb - Igaba_a - Isoma_ahp)) -\
                    Isoma_dpi_g_clip - ((1 + ((Igaba_a + Isoma_ahp - Isoma_pfb) /\
                    Isoma_dpi_tau)) * Isoma_mem)) / (tau_soma *\
                    (1+(Isoma_dpi_g_clip/Isoma_mem_clip))) : amp (unless refractory)

                    dIsoma_ahp/dt = (I0-Isoma_ahp_g_clip-Isoma_ahp)/(tau_soma_ahp*(1+(Isoma_ahp_g_clip/Isoma_ahp))) : amp # Adaptation current

                    Isoma_pfb = Isoma_pfb_gain/(1+exp(-(Isoma_mem_clip-Isoma_pfb_th)/Isoma_pfb_norm)) : amp               # Positive feedback current
                    Iin_clip = clip(Inmda + Iampa - Igaba_b + Isoma_const, I0,1*amp) : amp                 # Clip current to I0
                    Isoma_mem_clip = clip(Isoma_mem,I0,1*amp) : amp                                                       # Clip current to I0

                    Isoma_ahp_g_clip = Isoma_ahp_g*(Isoma_ahp>I0) + I0*(Isoma_ahp<=I0) : amp   # Shunt g current if Iahp goes to I0
                    Isoma_dpi_g_clip = Isoma_dpi_g*(Isoma_mem>I0) + I0*(Isoma_mem<=I0) : amp   # Shunt g current if Imem goes to I0
                    ...
                    # NMDA #######################################################
                    dInmda/dt = (I0 -Inmda - Inmda_g_clip)/(tau_nmda * ((Inmda_g_clip / Inmda) + 1)) : amp
                    ...
                    Inmda_g_clip = Inmda_g*(Inmda>I0) + I0*(Inmda<=I0) : amp            # Shunt g current if Inmda goes to I0
                    Inmda_thr : amp                                                     # NMDA voltage-gating threshold
                    ...
                    # AMPA #######################################################
                    dIampa/dt = (I0-Iampa - Iampa_g_clip)/(tau_ampa * ((Iampa_g_clip / Iampa) + 1)) : amp
                    ...
                    Iampa_g_clip = Iampa_g*(Iampa>I0) + I0*(Iampa<=I0) : amp            # Shunt g current if Iampa goes to I0
                    ...
                    dIgaba_b/dt = (I0-Igaba_b - Igaba_b_g_clip)/(tau_gaba_b * ((Igaba_b_g_clip / Igaba_b) + 1)) : amp
                    ...
                    Igaba_b_g_clip = Igaba_b_g*(Igaba_b>I0) + I0*(Igaba_b<=I0) : amp    # Shunt g current if Igaba_b goes to I0
                    ...
                    # GABA A - shunt #####################################################
                    dIgaba_a/dt =(I0-Igaba_a - Igaba_a_g_clip)/(tau_gaba_a * ((Igaba_a_g_clip / Igaba_a) + 1)) : amp
                    ...
                    Igaba_a_g_clip = Igaba_a_g*(Igaba_a>I0) + I0*(Igaba_a<=I0) : amp    # Shunt g current if Igaba_a goes to I0
